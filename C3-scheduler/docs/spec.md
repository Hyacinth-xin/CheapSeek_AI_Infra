...
下表列出各模型导出后实际使用的全部算子（opset 17）。选手的推理程序须能执行对应模型所用的算子。

| 模型 | 算子数 | 使用的算子 |
|------|:---:|------------|
| MLP | 3 | `Flatten`、`Gemm`、`Relu` |
| ResNet-18（简化） | 6 | `Add`、`Conv`、`Flatten`、`Gemm`、`GlobalAveragePool`、`Relu` |
| Transformer | 12 | `Add`、`Constant`、`Div`、`Erf`、`Gather`、`LayerNormalization`、`MatMul`、`Mul`、`Reshape`、`Softmax`、`Split`、`Transpose` |
| BigFormer | 13 | `Add`、`Constant`、`Div`、`Erf`、`Gather`、`Identity`、`LayerNormalization`、`MatMul`、`Mul`、`Reshape`、`Softmax`、`Split`、`Transpose` |

全部 18 种算子的并集：`Add`、`Constant`、`Conv`、`Div`、`Erf`、`Flatten`、`Gather`、`Gemm`、`GlobalAveragePool`、`Identity`、`LayerNormalization`、`MatMul`、`Mul`、`Relu`、`Reshape`、`Softmax`、`Split`、`Transpose`
...
- BigFormer 与 Transformer 结构相近，但 BigFormer 为双向注意力（无因果掩码），权重以外部数据文件存储；此外 BigFormer 图中含 `Identity` 节点，用于将同一权重张量别名给多个使用点，语义即直通复制（`output = input`），实现上直接透传输入即可
